...
import { Container, Desc, Heading, ImageCol, PreviewImgWrapper, PrizeRow, SubHeading, TextCol, ThumbnailGroup } from "./ProductDetails.styles";
import { HiddenRadio, InputGroup, SelectionLabel, SelectionRow, CtaBtn } from "../../components/Shared/ProductAttrBtn.styles";
...
class ProductDetails extends Component {
	...
	sortVariantIdProps = () => {
		...
		sortedObjKeys.forEach((key) => (variantID += `-(${key.toLowerCase()} - ${this.state.selectedAttributes[key].id.toLowerCase()})`));
	...
	addToCartHandler = (priceObj) => {
		...
		} else if (attributes.length > 0 && this.checkEmptyObject(this.state.selectedAttributes)) {
			alert("Before adding product to the Cart you must select attributes");
		} else {
	...
	render() {
		const { brand, name, description, gallery, attributes, prices, inStock } = this.state.product;
		...
		const { amount: calculatedPrice, currency: { symbol } = {} } = productPrice || {};
			...
			<Container>
						...
						{gallery?.length > 1 &&
							gallery?.map((imgUrl) => (
								<img
									...
									width={80}
									onClick={() => (this.previewImgRef.current.src = imgUrl)}
								/>
							))}
					</ThumbnailGroup>

					<PreviewImgWrapper>
						<img ref={this.previewImgRef} src={gallery?.[0]} alt={`${name} product preview image`} />
					</PreviewImgWrapper>
				</ImageCol>
				...
				<TextCol>
									...
									return (
										<InputGroup key={id} $isColorBox={attrId === "Color"}>
											<HiddenRadio
												...
												name={attrId}
												id={attrId + id}
												onChange={() => this.attributeHandler(attrId, item)}
											/>
											<SelectionLabel
												htmlFor={attrId + id}
												title={displayValue}
					...
					<PrizeRow>
						...
						<p className="prize">{symbol + calculatedPrice || ""}</p>
					</PrizeRow>

					<CtaBtn type="button" $inStock={inStock} onClick={() => this.addToCartHandler(productPrice)}>
						add to cart
					</CtaBtn>

					<Desc id="product-desc">
						{parse(description || "", {
							replace: ({ name }) => name && name === "h1" && <></>,
						})}
					</Desc>
				</TextCol>
			</Container>
